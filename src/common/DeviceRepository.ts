...
export class DeviceRepository {
  ...
  public async updateDevice(device: Partial<Device>) {
    ...
    try {
      const res = await session.writeTransaction((tx) =>
        tx.run(`MATCH (d:DEVICE {id: $id}) ${this._makeEditString(device, "d")} RETURN d`, {
          ...device,
          last_login: neo4j.types.DateTime.fromStandardDate(lastLoginDate),
        })
      );
  ...
  public async getNIdleDevices(n: number): Promise<Device[]> {
    ...
    try {
      const res = await session.readTransaction((tx) => tx.run('MATCH (d:DEVICE) WHERE d.status = "idle" RETURN d LIMIT $n', { n }));
